...
public class characterJump : MonoBehaviour
{
   ...
   // [SerializeField] movementLimiter moveLimit;
    ...
    [SerializeField, Range(30, 100)] private int dashStrength = 50;
   ...
   // private bool pressingJump;
    ...
    private void calculateGravity()
    {
        ...
        {
            if (onGround || !inJump || move.isWallSliding)
            {
                gravMultiplier = 1f;
            }
            else
    ...
    public void OnDash(InputAction.CallbackContext context)
    {
        ...
        if (context.started)
        {
            if (DashUnlocked)
            {
                if (lookingRight)
                {
                    rb.AddForce( new Vector2(1,0.1f) * dashStrength, ForceMode2D.Impulse);
                }
                else
                {
                    rb.AddForce(new Vector2(-1, 0.1f) * dashStrength, ForceMode2D.Impulse);
    ...
    public void ReloadAirJump()
    {
        //Used by the double jump powerup
        remainingAirJumps++;
    }
