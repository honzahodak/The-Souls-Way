...
public class PlayerDeath: MonoBehaviour
{
    ...
    public int maxCheckpoints = 1;

    [Header("Empty Souls")]
    public int maxEmptySouls = 0;
    ...
    public void IncreaseMaxCheckpoints(int howMany)
    {
        maxCheckpoints += howMany;
        for (int i = maxCheckpoints-howMany; i < maxCheckpoints; i++)
        {
            GameObject go = Instantiate(Resources.Load("Soul"), checkpointChrgesGameObject.transform.position, Quaternion.identity, checkpointChrgesGameObject.transform) as GameObject;
            checkpointCharges.Add(go.GetComponent<Image>());
            go.GetComponent<Image>().fillAmount = 0;
        }
    }
    ...
    {
        if(maxEmptySouls > 0)
            sr.color = new Color(1, 1, 1, Mathf.Clamp(1 - (float)emptySouls.Count / (float)maxEmptySouls, 0.05f, 1f));
